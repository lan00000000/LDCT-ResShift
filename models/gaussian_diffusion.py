...
class GaussianDiffusion:
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final = sample["sample"]
        with th.no_grad():
            out = self.decode_first_stage(sample, first_stage_model)
        return out
    ...
    def decode_first_stage(self, z, first_stage_model):
        # 如果没有第一阶段模型（VQGAN），说明就在像素空间，直接返回
        if first_stage_model is None:
            return z
        with th.no_grad():
            z = z.type(next(first_stage_model.parameters()).dtype)
    ...
    def encode_first_stage(self, y, first_stage_model, up_sample=False):
        ...
        with th.no_grad():
            ...
            return (z_y * self.scale_factor).type(y.dtype)
